...
class InstaBot:
    ...
    def __init__(self, config=None, **kwargs):
        ...
        self.startup = True
        self.media_to_like = []
        self.liked_medias = set()
        self.media_by_tag = []
        self.media_to_comment = []
        ...
        self.logger.info(
            "Instabot v{} started at {}:".format("2.0", now_time.strftime("%d.%m.%Y %H:%M")))
        self.prog_run = True
        self.next_iteration = {
            "Like": 0,
            "Unlike": 0,
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    try:
                        ...
                        print("exception in following", e.args)
                return resp
            except:
    ...
    def get_medias_from_recent_feed(self):
        ...
        try:
            ...
            self.logger.debug(f"Media in recent feed = {len(self.media_on_feed)}")

        except Exception as exc:
            logging.exception(exc)
            self.media_on_feed = []
        return self.media_on_feed
    ...
    def get_media_id_recent_feed(self):
        '''
            fetch recently posted medias.
            saves it in self.media_on_feed.
        :return:
        '''
        if self.login_status:
            now_time = datetime.datetime.now()
    ...
    def fetch_recent_media_recursive(self, count, __cursor=None):
        '''
            fetch recently posted medias.
            The no of fetched media is less or equal to count.
            saves it in self.media_on_feed.
        :return:
        '''
        print("Fetch %d recent medias  -->" % count, end='')
        if self.login_status:

            if self.login_status == 1:
                url_tag = 'https://www.instagram.com/?__a=1'
                if __cursor is not None:
                    url_tag += '&max_id=' + __cursor

                try:
                    r = self.s.get(url_tag)
                    all_data = json.loads(r.text)

                    media = all_data['graphql']['user']['edge_web_feed_timeline']['edges']
                    self.media_on_feed.extend(media)
                    count -= len(self.media_on_feed)
                    print("Fetched %d medias" % len(media))
                    if count > 0:
                        if all_data['graphql']['user']['edge_web_feed_timeline']['page_info']['has_next_page']:
                            cursor = all_data['graphql']['user']['edge_web_feed_timeline']['page_info']['end_cursor']
                            return self.fetch_recent_media_recursive(count, cursor)
                    else:
                        return True
                except    Exception as e:
                    print(e)
